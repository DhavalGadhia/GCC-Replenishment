...
#   Cover Weeks = SOH / ROS / 7      -> weeks of stock the store is holding
#
...
def build_universe(sales, depth, soh_ag, in_transit, country_map,
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        cover_d = np.where(u["ROS"] > 0,
                           u["SOH"].clip(lower=0) / u["ROS"].replace(0, np.nan),
                           np.inf)
    ...
    if ros_adjust:
        ...
        u.loc[u["ROS"].notna() & bad_soh, "Target Stock Basis"] = TARGET_BASIS_NEGSOH
...
with st.container(border=True):
    ...
    ros_on = st.toggle(
        ...
        help="Cover Weeks = SOH / ROS / 7, where ROS is the store's own observed "
