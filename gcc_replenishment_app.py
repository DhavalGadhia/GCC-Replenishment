...
def convert_df_to_excel(df: pd.DataFrame, sheet_name: str = "Sheet1") -> bytes:
    """
    Writes df to a formatted .xlsx (bold header, sensible column widths).

    IMPORTANT: column width is estimated from a small SAMPLE of rows, not a
    scan of every cell. Scanning every cell in every column (the original
    approach) is fine at a few hundred rows but becomes the single biggest
    bottleneck once a file reaches tens/hundreds of thousands of rows -
    this was the main cause of the app hanging on a large upload.
    """
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for col_idx, col_name in enumerate(df.columns, start=1):
            ...
            if sample_size:
                max_len = sample[col_name].astype(str).map(len).max()
            else:
                max_len = 0
...
def _read_excel_bytes(file_bytes: bytes, file_name: str) -> pd.DataFrame:
    """
    Cached so re-running the app (any button click, any widget touch) does
    NOT re-read every uploaded file from scratch each time - Streamlit
    reruns the whole script on every interaction, and without this cache
    every rerun was re-parsing potentially large Excel files for no reason.
    Keyed on the file's raw bytes, so a genuinely new/changed file still
    gets read fresh.
    """
...
def safe_read_uploaded_files(uploaded_files):
    """Reads each uploaded file, catching bad/corrupt files individually so
    one broken upload can't crash the whole app."""
    files_data = []
    read_errors = []
    for f in uploaded_files:
        try:
            df = _read_excel_bytes(f.getvalue(), f.name)
            files_data.append((f.name, df))
...
for key in ("final_df", "detail_df", "plan_bytes", "detail_bytes"):
        ...
        if files_data:
            with st.expander(f"Preview {len(files_data)} uploaded file(s)", expanded=False):
                for name, fdf in files_data:
                    st.markdown(f"**{name}** - {len(fdf):,} row(s)")
                    st.dataframe(fdf.head(20), use_container_width=True)

            if st.button("Run Replenishment", type="primary", disabled=not files_data):
                try:
                    ...
                    if n_rows > 50_000:
                        st.info(
                            f"Processing {n_rows:,} rows - this may take a minute or two for a "
                            f"file this size."
                        )

                    with st.spinner(f"Running replenishment logic on {n_rows:,} rows..."):
                    ...
                    # Expected, validation-type failures (missing columns,
                    # duplicate rows) - message is already written to be
                    # read directly by the planner.
                    for key in ("final_df", "detail_df", "plan_bytes", "detail_bytes"):
                        st.session_state[key] = None
                    st.error(str(e))

                except Exception as e:
                    # Anything unexpected (corrupt data, memory pressure,
                    # etc.) - caught here so the app shows a readable error
                    # instead of crashing outright. Technical details are
                    # tucked away but available if you need to share them.
                    for key in ("final_df", "detail_df", "plan_bytes", "detail_bytes"):
                        st.session_state[key] = None
                    st.error(
                        f"Something went wrong while processing this file "
                        f"({type(e).__name__}: {e}). Try again, or check the technical "
                        f"details below if it keeps happening."
                    )
...
if st.session_state.final_df is not None:
    ...
    with st.expander("Calculation Detail - Audit Trail", expanded=False):
        ...
        # Styling (colored Grade/Reserve badges) also has real overhead on
        # very large tables - skip it past a size threshold so the page
        # itself stays responsive; the download still has full detail.
        if len(detail_df) <= 20_000:
            st.dataframe(style_detail_table(detail_df), use_container_width=True, hide_index=True)
        else:
            st.caption(
                f"{len(detail_df):,} rows - showing plain (unstyled) for performance. "
                f"Full detail is in the download above."
            )
