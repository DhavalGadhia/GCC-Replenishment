...
def build_universe(sales, depth, soh_ag, in_transit, country_map,
    ...
    def _band(col, idx, default):
        return u["Country"].map(
            {c: v[idx] for c, v in bands.items()}).astype(float).fillna(default)

    u["Increase Up To (wks)"] = _band("inc", 0, 6)
    u["Decrease From (wks)"]  = _band("dec", 1, 16)
    u["Stop From (wks)"]      = _band("stop", 2, 25)
    inc_pct = _band("ip", 3, 30)
    dec_pct = _band("dp", 4, 30)
    ...
    if ros_adjust:
        ...
        wk = u["Cover Weeks"]
        up   = usable & (wk <= u["Increase Up To (wks)"])
        stop = usable & (wk >= u["Stop From (wks)"])
        down = usable & ~stop & (wk >= u["Decrease From (wks)"])
...
with st.container(border=True):
    ...
    with band_l:
        ...
        cover_bands, band_warn = {}, []
        for _, rr in band_edit.iterrows():
            def _n(col, dflt):
                x = pd.to_numeric(rr[col], errors="coerce")
                return float(dflt) if pd.isna(x) else float(x)
            inc_to = _n("Increase Up To (wks)", 6)
            dec_fr = _n("Decrease From (wks)", 16)
            stop_fr = _n("Stop From (wks)", 25)
            if not (inc_to < dec_fr <= stop_fr):
                band_warn.append(str(rr["Country"]))
                inc_to, dec_fr, stop_fr = 6, 16, 25
            cover_bands[rr["Country"]] = (inc_to, dec_fr, stop_fr,
                                          _n("Increase %", 30), _n("Decrease %", 30))
        if band_warn:
            st.warning("Thresholds must rise left to right (increase < decrease "
                       "<= stop). Reverted to defaults for: " + ", ".join(band_warn))
    ...
    with band_r:
        ...
        ref = max(cover_bands.items(), key=lambda kv: kv[1][2]) if cover_bands \
            else ("", (6, 16, 25, 30, 30))
        i0, d0, s0, ip, dp = ref[1]
        span = max(s0 * 1.4, 1)
        seg = [("inc", 0, i0, f"+{ip:.0f}%"), ("keep", i0, d0, "hold"),
               ("dec", d0, s0, f"&minus;{dp:.0f}%"), ("stop", s0, span, "stop")]
        bars = "".join(
        ...
        st.html(
            ...
            f'<div class="bd-labs"><span>0</span><span>{i0:.0f}</span>'
            f'<span>{d0:.0f}</span><span>{s0:.0f} wks+</span></div>'
