...
RUN_REPLENISHMENT, RUN_CONNECTION = "Replenishment", "Connection"
...
def inject_theme():
    ...
    .st-key-run_Replenishment, .st-key-run_Connection{ margin-top:-30px !important; }
    .st-key-pick_Pack button, .st-key-pick_Loose button,
    .st-key-run_Replenishment button, .st-key-run_Connection button{
    ...
    .st-key-run_Replenishment button:disabled, .st-key-run_Connection button:disabled{
...
with st.container(border=True):
    ...
    RUN_TABS = [
        (RUN_REPLENISHMENT, "Replenishment", "top up stores already trading the option",
         "&#8635;"),
        (RUN_CONNECTION, "Connection", "place an option into a store for the first time",
         "&#43;"),
    ]
    ...
    for col, (m, label, sub, glyph) in zip(rt_cols, RUN_TABS):
        with col:
            ...
            if st.button("Selected" if live else f"Use {label}",
                         key=f"run_{m}", use_container_width=True,
