...
def prepare(frames, active_wh, reserve_map=None):
    ...
    # so its entire available quantity is allocatable.
    rmap = reserve_map or {}
    wms_ag["Reserve %"] = wms_ag["WH"].map(rmap).fillna(0.0)
    wms_ag["Working Pool"] = (
        wms_ag["Opening"] * (1 - wms_ag["Reserve %"] / 100.0)
...
def allocate(u, wms_ag):
    ...
    pools = dict(zip(
        ...
        wms_ag["Working Pool"].to_numpy().astype(int),
    ))
...
def run_engine(frames, active_wh, pack_thresholds=None, default_targets=None,
    ...
    sales, depth, soh_ag, in_transit, wms_ag, cmap, warns = prepare(
        frames, active_wh, reserve_map)
    ...
    wh_summary = wh_summary[["WH", "WH Name", "Opening", "Reserve %", "Reserve Held",
                             "Working Pool", "Shipped", "Unused Pool",
                             "Pool Utilisation %"]]
