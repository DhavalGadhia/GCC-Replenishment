...
# CONFIDENCE WINDOW
# A store that has only had an option for a few days has too little history for
# a trustworthy rate of sale - one sale in 15 days implies 700 days of cover.
# Rather than discard those readings at an arbitrary cut-off, each store is
# given a head start of k days trading at the option's proven rate:
#
#   effective ROS = (units sold + k x option benchmark) / (days on sale + k)
#
# At k=21 a store's own record carries equal weight to the benchmark after 21
# days, and dominates thereafter. k=0 disables the blend entirely.
DEFAULT_CONFIDENCE_DAYS = 21
BENCHMARK_MATURE_DAYS = 60     # a store needs this much history to inform the benchmark
...
# Negative stock on hand is a data error, not an empty shelf. Left unadjusted
# rather than read as an urgent stockout.
TARGET_BASIS_NEGSOH   = "Not adjusted - negative stock on hand (data issue)"
TARGET_BASIS_LAUNCH   = "Launch period - DEPTH target (first {days:.0f} trading days)"

# LAUNCH GRACE PERIOD
# A newly launched option has no meaningful rate of sale yet, and its first
# days on the floor are not representative. For this many trading days the
# rate-of-sale adjustment is skipped entirely and the DEPTH target is used as
# planned. After it, the normal logic resumes automatically.
#   "store"   - counted from when THIS store received the option (default)
#   "option"  - counted from the option's FIRST launch anywhere in the network
# Store scope is the default because rollouts are heavily staggered - a median
# of 92 days between the first and last store receiving an option - so a store
# can be on day one with an option the network launched months ago. Its rate of
# sale is just as unreliable as a true new launch.
DEFAULT_LAUNCH_GRACE_DAYS = 7
LAUNCH_SCOPE_OPTION, LAUNCH_SCOPE_STORE = "option", "store"
DEFAULT_LAUNCH_SCOPE = LAUNCH_SCOPE_STORE
...
def prepare(frames, active_wh, reserve_map=None, wh_names=None):
    ...
    if {"NetSalesQty", "FirstReceived"} <= set(soh_norm.columns):
        ...
        # Network launch age of the option: days since the FIRST store anywhere
        # received it. Computed across every store holding it, not just the
        # in-scope ones, so a narrow run still dates the launch correctly.
        opt_age = ros.groupby("Option")["Days On Sale"].max().rename("Option Age Days")
        ros = ros.join(opt_age, on="Option")

        # Benchmark rate per Option, taken across stores with enough history.
        # Computed on ALL stores holding the option, not just the in-scope ones,
        # so a thinly-scoped run still gets a solid reference rate.
        mature = ros[ros["Days On Sale"] >= BENCHMARK_MATURE_DAYS]
        bench = mature.groupby("Option")["ROS"].median().rename("Option ROS Benchmark")
        ros = ros.join(bench, on="Option")
    else:
        warnings.append(
        ...
        ros = pd.DataFrame(columns=["Option", "Store", "ROS", "Days On Sale",
                                    "Lifetime Sold", "Option ROS Benchmark",
                                    "Option Age Days"])
...
def build_universe(sales, depth, soh_ag, in_transit, country_map,
                   ...
                   confidence_days=DEFAULT_CONFIDENCE_DAYS,
                   launch_grace_days=DEFAULT_LAUNCH_GRACE_DAYS,
                   launch_scope=DEFAULT_LAUNCH_SCOPE):
    """
    ...
    # much history the store actually has. Fades out on its own as days build up.
    kdays = float(DEFAULT_CONFIDENCE_DAYS if confidence_days is None else confidence_days)
    if "ROS" in u.columns:
        if "Option ROS Benchmark" not in u.columns:
            u["Option ROS Benchmark"] = np.nan
        blendable = (kdays > 0) & u["ROS"].notna() & u["Option ROS Benchmark"].notna() \
                    & u["Days On Sale"].notna()
        u["Effective ROS"] = u["ROS"]
        u.loc[blendable, "Effective ROS"] = (
            (u.loc[blendable, "Lifetime Sold"].clip(lower=0)
             + kdays * u.loc[blendable, "Option ROS Benchmark"])
            / (u.loc[blendable, "Days On Sale"] + kdays))
    else:
        u["Effective ROS"] = np.nan

    # ---- minimum rate of sale ------------------------------------------
    # Applied only where a ROS figure exists, so lines with no ROS are never
    # silently dropped by it.
    n_slow = 0
    # Tested on the RAW rate, not the blended one: this filter is about whether
    # a store is actually still selling. A dormant store must not stay in scope
    # by borrowing its option's benchmark.
    ...
    if ros_adjust and "ROS" in u.columns:
        eff = u["Effective ROS"]
        has_ros = eff.notna() & u["Target Cover Days"].notna()
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            cover = np.where(eff > 0, soh_pos / eff.replace(0, np.nan), np.inf)
        u["Cover Days"] = np.where(has_ros, cover, np.nan)

        # Negative stock on hand means the stock record is wrong, not that the
        # shelf is empty. Clipping it to zero would read as zero cover and
        # trigger a 30% increase plus a full stockout order - a systems error
        # amplified into an urgent replenishment. Leave these on their DEPTH
        # target and flag them instead.
        bad_soh = u["SOH"] < 0

        # Launch grace: a brand new option keeps its planned DEPTH target for
        # the first few trading days rather than being judged on a handful of
        # transactions.
        grace_days = float(DEFAULT_LAUNCH_GRACE_DAYS if launch_grace_days is None
                           else launch_grace_days)
        if grace_days > 0:
            age_col = ("Option Age Days"
                       if launch_scope == LAUNCH_SCOPE_OPTION
                       and "Option Age Days" in u.columns
                       else "Days On Sale")
            in_launch = u[age_col].notna() & (u[age_col] < grace_days)
        else:
            in_launch = pd.Series(False, index=u.index)

        over = has_ros & ~bad_soh & ~in_launch & (u["Cover Days"] >= u["Target Cover Days"])
        under = has_ros & ~bad_soh & ~in_launch & (u["Cover Days"] < u["Target Cover Days"])
        ...
        u.loc[has_ros & bad_soh, "Target Stock Basis"] = TARGET_BASIS_NEGSOH
        u.loc[in_launch, "Target Stock Basis"] = TARGET_BASIS_LAUNCH.format(days=grace_days)
    else:
        u["Cover Days"] = np.nan
...
def run_engine(frames, active_wh, pack_thresholds=None, default_targets=None,
               ...
               min_ros=DEFAULT_MIN_ROS, confidence_days=DEFAULT_CONFIDENCE_DAYS,
               launch_grace_days=DEFAULT_LAUNCH_GRACE_DAYS,
               launch_scope=DEFAULT_LAUNCH_SCOPE,
               progress_cb=None):
    def report(f, label):
    ...
    u, n_defaulted, n_slow = build_universe(sales, depth, soh_ag, in_transit,
                                            ...
                                            ros_tbl, min_ros, confidence_days,
                                            launch_grace_days, launch_scope)
    n_launch = int(u["Target Stock Basis"].str.startswith("Launch period").sum()) \
        if "Target Stock Basis" in u.columns else 0
    if n_launch:
        warns.append(
            f"{n_launch:,} option x store line(s) are inside the {launch_grace_days:g}-day "
            f"launch window and kept their DEPTH target - the rate-of-sale adjustment "
            f"does not apply to them yet.")
...
with st.container(border=True):
    ...
    with ros_r:
        if ros_on:
            ...
            confidence_days = st.number_input(
                "Confidence window (days)", min_value=0, max_value=120,
                value=int(DEFAULT_CONFIDENCE_DAYS), step=7,
                help="A store with little history has an unreliable rate of sale - "
                     "one sale in 15 days implies 700 days of cover. Each store is "
                     "given a head start of this many days at the option's proven "
                     "rate, so its own record takes over gradually. 0 disables it.")
            if confidence_days > 0:
                bars = "".join(
                    f'<div class="cw-row"><span class="cw-d">{dd}d</span>'
                    f'<span class="cw-bar">'
                    f'<span class="cw-own" style="width:{dd/(dd+confidence_days)*100:.0f}%">'
                    f'</span></span>'
                    f'<span class="cw-p">{dd/(dd+confidence_days)*100:.0f}%</span></div>'
                    for dd in (4, 21, 60, 200))
                st.html(
                    f'<div class="cw"><div class="cw-head">weight given to a store&rsquo;s '
                    f'OWN sales, by how long it has held the option</div>{bars}'
                    f'<div class="cw-foot">the remainder comes from the option&rsquo;s '
                    f'benchmark rate across mature stores</div></div>')

            st.divider()
            st.markdown("**Launch grace period**")
            launch_grace_days = st.number_input(
                "Trading days before the rate-of-sale rule applies", 0, 90,
                int(DEFAULT_LAUNCH_GRACE_DAYS), 1,
                help="A newly launched option has no meaningful rate of sale yet. "
                     "For this many trading days it keeps its planned DEPTH target; "
                     "afterwards the adjustment applies automatically. 0 disables it.")
            launch_scope = st.radio(
                "Counted from", [LAUNCH_SCOPE_STORE, LAUNCH_SCOPE_OPTION],
                index=0, horizontal=True,
                format_func=lambda k: ("when this store received it"
                                       if k == LAUNCH_SCOPE_STORE
                                       else "the option's first launch anywhere"),
                help="Rollouts are staggered, so each store starts its own clock from "
                     "the day it received the option. Switch to the network launch "
                     "date only if you want the grace period to expire everywhere at "
                     "the same time.")
            if launch_grace_days > 0:
                st.html(
                    f'<div class="lg">'
                    f'<div class="lg-track">'
                    f'<span class="lg-grace" style="width:{min(100, launch_grace_days/60*100):.0f}%">'
                    f'</span></div>'
                    f'<div class="lg-labs"><span>day 0<br><i>store receives</i></span>'
                    f'<span class="lg-mid">DEPTH target held</span>'
                    f'<span>day {launch_grace_days:g}<br><i>ROS resumes</i></span></div></div>')
            st.divider()
            ...
            min_ros = st.number_input(
                ...
                step=0.01, format="%.2f",
                help="Lines selling slower than this are excluded. Tested on the "
                     "store's OWN observed rate, not the blended one, so a dormant "
                     "store cannot stay in scope by borrowing its option's "
        ...
        if st.button("Run Replenishment", type="primary", disabled=not frames):
            ...
            try:
                final, detail, wh_summary, warns, n_defaulted, secs = run_engine(
                    ...
                    increase_pct, decrease_pct, ros_on, min_ros, confidence_days,
                    launch_grace_days, launch_scope,
                    progress_cb)
