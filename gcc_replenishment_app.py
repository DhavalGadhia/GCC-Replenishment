...
def run_engine(frames, active_wh, pack_thresholds=None, default_targets=None,
    ...
    final = pd.DataFrame({
        ...
        # Whole packs behind the quantity. Zero on a loose run, where stock
        # ships as individual units and there is no pack to count.
    ...
    if consolidate and len(final):
        final, n_rolled = consolidate_regional(final, hit["Country"].to_numpy(),
                                               wh_names)
        if n_rolled:
            warns.append(
