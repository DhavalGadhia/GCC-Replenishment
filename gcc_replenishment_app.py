...
DEFAULT_INCREASE_PCT = 30      # applied when under-covered
DEFAULT_DECREASE_PCT = 10      # applied when over-covered
ROS_ADJUST_ON = True
...
def _locate_soh_blocks(rows):
    ...
    name_row = meta[-2] if len(meta) >= 2 else meta[-1]
    ctry_row = meta[0]
...
with st.container(border=True):
    ...
    with ros_r:
        if ros_on:
            increase_pct = st.slider("Increase % when UNDER-covered", 0, 100,
                                     DEFAULT_INCREASE_PCT, 5, format="%d%%")
            decrease_pct = st.slider("Decrease % when OVER-covered", 0, 100,
                                     DEFAULT_DECREASE_PCT, 5, format="%d%%")
            ...
            span = max(increase_pct, decrease_pct, 1)
            st.html(
                f'<div class="skew">'
                f'<div class="skew-row"><span class="skew-lab">over-covered</span>'
                f'<span class="skew-bar"><span class="sk-dn" '
                f'style="width:{50*decrease_pct/span:.0f}%"></span></span>'
                f'<span class="skew-val">&minus;{decrease_pct}%</span></div>'
                f'<div class="skew-row"><span class="skew-lab">under-covered</span>'
                f'<span class="skew-bar"><span class="sk-up" '
                f'style="width:{50*increase_pct/span:.0f}%"></span></span>'
                f'<span class="skew-val">+{increase_pct}%</span></div></div>')
        else:
            increase_pct = decrease_pct = 0
            min_ros = 0.0
