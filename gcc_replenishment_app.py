...
def build_basics_groups(soh, depth, country_map, default_targets, allowed=None):
    ...
    d = soh.assign(
        ...
        Cat=_S(soh.get("Budget Category", pd.Series("", index=soh.index))),
        Col=_S(soh.get("Item Colour", pd.Series("", index=soh.index))),
        Seas=_S(soh.get("Seas Desc", pd.Series("", index=soh.index))),
    )
    if allowed is not None:
    ...
    d = (d.groupby(["Store", "Group", "Cat", "Col", "Option"], as_index=False)
...
def basics_need(members, depth, in_transit, default_targets, pack_size_map,
                pack_thresholds, mode=DEFAULT_MODE):
    """
    ...
    """
    m = members.merge(
        depth.rename(columns={"Target": "Cap"}), on=["Option", "Store"], how="left")
    m = m.merge(in_transit[["Option", "Store", "InTransit"]],
                on=["Option", "Store"], how="left")
    m["InTransit"] = pd.to_numeric(m["InTransit"], errors="coerce").fillna(0).clip(lower=0)

    m = m.sort_values(["Store", "Group", "SeasonKey", "Option"], kind="mergesort")
    grp = m.groupby(["Store", "Group", "Cat", "Col", "Country"], as_index=False)

    g = grp.agg(**{"SOH": ("SOH", "sum"), "Sold": ("Sold", "sum"),
                   "InTransit": ("InTransit", "sum"),
                   "Members": ("Option", list), "Seasons": ("Seas", list)})
    # Cap: last non-null in season order = newest member carrying one.
    caps = (m.dropna(subset=["Cap"])
              .groupby(["Store", "Group"], as_index=False)["Cap"].last())
    g = g.merge(caps, on=["Store", "Group"], how="left")
...
def _basics_audit(row):
    return {"Budget Category": row.Cat, "Item Colour": row.Col, "Store": row.Store,
            "Country": row.Country, "Cap": row.Cap, "Cap Source": row._asdict().get("Cap Source", ""),
...
def run_engine(frames, active_wh, pack_thresholds=None, default_targets=None,
    ...
    if run_type == RUN_BASICS:
        ...
        members, n_unres = build_basics_groups(
        ...
        g = basics_need(members, depth, in_transit, default_targets,
                        pack_size_map, pack_thresholds, mode)
        warns.append(
