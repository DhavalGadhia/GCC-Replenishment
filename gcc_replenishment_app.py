...
def build_connection_universe(depth, soh_ag, in_transit, sales, country_map,
    ...
    # The country pack-rounding grid is a replenishment control and is not
    # applied here. A connection still has to ship whole packs, so the DEPTH
    # target is taken to the nearest whole pack on a plain 50% rule.
    u["Pack Threshold %"] = FALLBACK_THRESHOLD_PCT
    ...
    u["Rounded Need"] = round_to_pack_vec(u["Target"], u["Pack Threshold %"],
                                          u["Pack Size Used"])
    u["Total Pipeline"] = 0
...
def allocate_connection(u, wms_ag, method=DEFAULT_CONN_METHOD, mode=DEFAULT_MODE,
    ...
    Stores are ordered by their own overall trading volume, biggest first, so
    that when stock runs short the busiest stores are served first. There is no
    per-option sales rank to use - by definition none of these stores has ever
    sold the option.

    ROUND ROBIN issues one pack per store per pass and repeats, so breadth
    comes before depth. UPFRONT fills each store's whole target in turn.
    """
    form_code = "P" if mode == MODE_PACK else "L"
    form_name = "Pack" if mode == MODE_PACK else "Loose"
    cprio = country_priority or COUNTRY_PRIORITY
    ...
    u = u.sort_values(["Option", "Store Volume", "Store"],
        ...
        if form_code == "P":
            for wh in (cprio.get(countries[i]) or []):
                ...
                if rr:
                    r = max(1, int(rr))
                    break
        unit[i] = r if (method == CONN_ROUND_ROBIN and form_code == "P") else needs[i]
    ...
    for i in range(n):
        ...
        remark[i] = (f"{tag}: first placement into {whn}-served store, {qdesc} "
                     f"({fill}) via {how}. Store rank {u['Store Rank'].to_numpy()[i]} "
                     f"for this option.")
