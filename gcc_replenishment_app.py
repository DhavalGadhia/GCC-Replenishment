...
def _csv_head(src, nrows=6):
    _seek0(src)
    return pd.read_csv(src, header=None, nrows=nrows, dtype=str,
                       sep=None, engine="python", on_bad_lines="skip")


def _find_csv_header(buf, signature, limit=10):
    """
    Locate the header row in a delimited file by looking for the row that
    carries the file's signature columns. A CSV export usually drops the blank
    preamble rows an Excel export keeps, so the header row cannot be assumed.
    """
    head = _csv_head(buf, nrows=limit)
    for i in range(len(head)):
        cells = {str(c).strip() for c in head.iloc[i] if pd.notna(c)}
        if signature <= cells:
            return i
    return 0


def read_flat_csv(buf, kind):
    """Same column selection as read_flat, but for a delimited text file."""
    spec = FILE_SPECS[kind]
    hdr = _find_csv_header(buf, spec["signature"])
    _seek0(buf)
    df = pd.read_csv(buf, skiprows=hdr, sep=None, engine="python",
...
def read_soh_pivot_csv(buf):
    ...
    r1, r2, r3, r4 = (list(raw.iloc[i]) for i in range(4))
    r1 = [None if pd.isna(v) else v for v in r1]
    r3 = [None if pd.isna(v) else v for v in r3]
    r4 = [None if pd.isna(v) else v for v in r4]
    n = len(r4)
    attr_end = 0
...
def identify_file(buf, name=""):
    ...
    if _is_csvlike(name):
        try:
            ...
            return None
        seen = {str(c).strip() for c in head.to_numpy().ravel() if pd.notna(c)}
...
def run_engine(frames, active_wh, pack_thresholds=None, default_targets=None,
               ...
               wh_names=None, progress_cb=None):
    def report(f, label):
        if progress_cb:
            progress_cb(f, label)
    ...
    (sales, depth, soh_ag, in_transit, wms_ag, cmap, store_names, budget_cat,
     pack_size_map, warns) = prepare(frames, active_wh, reserve_map, wh_names)
        ...
        if st.button("Run Replenishment", type="primary", disabled=not frames):
            ...
            try:
                final, detail, wh_summary, warns, n_defaulted, secs = run_engine(
                    ...
                    st.session_state.wh_registry, progress_cb)
