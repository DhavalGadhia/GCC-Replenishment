...
def allocate_basics(groups, wms_ag, wh_transit, mode=DEFAULT_MODE,
    ...
    g = groups.sort_values(["Group", "Sold", "Store"],
    ...
    for row in g.itertuples(index=False):
        ...
        psize = float(row._asdict().get("Pack Size", 1) or 1)
            ...
            for opt in members:
                ...
                for ci, wh in enumerate(chain):
                    ...
                    if ship > 0:
                        pool[k] = avail - ship
                        need -= ship
                        legs.append(_basics_leg(row, opt, wh, ship, "Pack",
                    ...
                    if psize > 0 and 0 < need < psize:
                        ...
                        if need / psize >= t and pool.get(k, 0.0) >= psize:
                            pool[k] -= psize
                            legs.append(_basics_leg(row, opt, wh, psize,
                                                    "Pack (rounded up)",
                                                    item_key.get(k, ""), names))
...
def _basics_leg(row, opt, wh, qty, kind, item_key, names):
