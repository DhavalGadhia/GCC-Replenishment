...
def _spec_fingerprint():
    ...
    payload = json.dumps(
        {k: {"cols": v["cols"], "header_row": v["header_row"],
         ...
         for k, v in FILE_SPECS.items()}, sort_keys=True)
...
def read_flat(buf, kind):
    ...
    for _ in range(spec["header_row"] - 1):
        next(it)
    hdr = list(next(it))
    idx = {str(c).strip(): i for i, c in enumerate(hdr) if c is not None}
...
def load_sources(file_payloads, spec_version):
    """
    file_payloads: tuple of (name, bytes). Cached on the raw bytes AND the
    spec fingerprint, so a rerun never re-parses the workbooks but a change to
    FILE_SPECS does force a fresh read.
...
def prepare(frames, active_wh, reserve_map=None):
    ...
    return (sales, depth, soh_ag, in_transit, wms_ag, country_map,
            store_names, budget_cat, pack_size_map, warnings)
...
def allocate(u, wms_ag, mode=DEFAULT_MODE):
    """
...
def run_engine(frames, active_wh, pack_thresholds=None, default_targets=None,
               reserve_map=None, mode=DEFAULT_MODE, progress_cb=None):
    def report(f, label):
        if progress_cb:
            progress_cb(f, label)
    ...
    (sales, depth, soh_ag, in_transit, wms_ag, cmap, store_names, budget_cat,
     pack_size_map, warns) = prepare(frames, active_wh, reserve_map)
    ...
    detail, pool_own, pool_inb = allocate(u, wms_ag, mode)
...
with st.container(border=True):
    ...
    rf_ctl, rf_vis = st.columns([1, 1.35])

    with rf_ctl:
        refill_mode = st.radio(
            "Replenish in", options=[MODE_PACK, MODE_LOOSE],
            index=[MODE_PACK, MODE_LOOSE].index(DEFAULT_MODE),
            label_visibility="collapsed",
            help="Pack ships whole packs of each option's own Pack Ratio from the WMS "
                 "extract. Loose ships individual eaches. A run is one or the other.",
        )

    with rf_vis:
        cards = []
        for m, label, sub, glyph in [
            ...
            live = refill_mode == m
            cards.append(
                f'<div class="form-card {"on" if live else "off"}">'
        ...
        note = ("Pack size is taken per option from the WMS <b>Pack Ratio</b> column "
                ...
                "country thresholds below do not affect a loose run.")
        st.html(f'<div class="forms">{"".join(cards)}</div>'
                f'<div class="vault-note">{note}</div>')
...
with st.container(border=True):
    ...
    if uploads:
        ...
        with st.spinner("Reading and identifying files..."):
            frames, timings, read_warnings = load_sources(payload, SPEC_VERSION)
        ...
        if st.button("Run Replenishment", type="primary", disabled=not frames):
            ...
            try:
                final, detail, wh_summary, warns, n_defaulted, secs = run_engine(
                    ...
                    reserve_map, refill_mode, progress_cb)
