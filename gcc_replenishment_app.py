...
with st.container(border=True):
    ...
    st.markdown('<p class="sec-sub">Defaults apply automatically. Change these only if you '
                'want to override them for this run.</p>', unsafe_allow_html=True)
    ...
    with left:
        ...
        st.dataframe(
            pd.DataFrame([{"Country": c, **{f"Priority {i+1}": GRID_WH[w]
                                            for i, w in enumerate(chain)}}
                          for c, chain in sorted(COUNTRY_PRIORITY.items())]).fillna("-"),
            use_container_width=True, hide_index=True)

        st.markdown("**Source warehouses in play**")
        active_wh = st.multiselect(
            "Warehouses", options=list(GRID_WH.keys()),
            default=list(GRID_WH.keys()),
            format_func=lambda c: f"{GRID_WH[c]} ({c})",
            label_visibility="collapsed",
            help="Only stock in these warehouses is available to allocate. E-commerce, "
                 "virtual and reservation locations are excluded unless added to the grid "
                 "in WH_REGION_PRIORITY.")
        st.caption(f"A {int(WMS_RESERVE_PCT*100)}% reserve is held at every warehouse and is "
                   f"never released. Inbound inter-warehouse transit is added to the receiving "
                   f"warehouse; nothing is deducted from the sender.")

    with right:
        st.markdown("**Pack rounding thresholds**")
        customise = st.toggle("Customise thresholds for this run", value=False,
                              help="Off: defaults below. On: edit any percentage before running.")
        ...
        st.markdown("\n".join(
            f"- **{', '.join(cs)}** at **{p:.0f}%** &rarr; a remainder of "
            f"**{PACK_SIZE*p/100:.1f}+ units** rounds up to the next pack of {PACK_SIZE}"
            for p, cs in sorted(groups.items())))

        st.divider()
        st.markdown("**Default target stock**")
        st.caption("Applied when an Option + Store combination has no row in the DEPTH file, "
                   "so a selling store is never skipped for lack of a target.")
        tgt_customise = st.toggle("Customise default targets for this run", value=False)
        ...
        if tgt_customise:
            ...
            if st.button("Reset targets to defaults", use_container_width=True):
